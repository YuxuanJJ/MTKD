...
def create_lmdb_for_bvidvc():
    ...
    folder_path = 'datasets/REDS/train_sharp_bicubic'
    lmdb_path = 'datasets/REDS/train_sharp_bicubic_with_val.lmdb'
    img_path_list, keys = prepare_keys_bvidvc(folder_path)
    make_lmdb_from_imgs(folder_path, lmdb_path, img_path_list, keys, multiprocessing_read=True)


def prepare_keys_bvidvc(folder_path):
    """Prepare image path list and keys for REDS dataset.
...
def create_lmdb_for_vid4():
    # GT
    folder_path = 'datasets/Vid4/GT'
    lmdb_path = 'datasets/Vid4/GT_with_val.lmdb'
    img_path_list, keys = prepare_keys_vid4(folder_path)
    make_lmdb_from_imgs(folder_path, lmdb_path, img_path_list, keys, multiprocessing_read=True)

    # BIx4
    folder_path = 'datasets/Vid4/BIx4'
    lmdb_path = 'datasets/Vid4/BIx4_with_val.lmdb'
    img_path_list, keys = prepare_keys_vid4(folder_path)
    make_lmdb_from_imgs(folder_path, lmdb_path, img_path_list, keys, multiprocessing_read=True)


def prepare_keys_vid4(folder_path):
    """Prepare image path list and keys for REDS dataset.
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Options: 'DIV2K', 'REDS', 'Vimeo90K', 'Vid4' You may need to modify the corresponding configurations in codes."))
