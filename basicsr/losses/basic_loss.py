...
@weighted_loss
def sphere_loss(x, y, eps=1e-12):
    batch_dot = (x * y).sum(-1)
    batch_dot = torch.clamp(batch_dot, -1 + eps, 1 - eps)
...
@LOSS_REGISTRY.register()
class MultiLevelL1Loss(nn.Module):
    """L1 (mean absolute error, MAE) loss.
    ...
    def __init__(self, loss_weight=1.0, reduction='mean'):
        super(MultiLevelL1Loss, self).__init__()
        if reduction not in ['none', 'mean', 'sum']:
            raise ValueError(f'Unsupported reduction mode: {reduction}. Supported ones are: {_reduction_modes}')
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        Args:
            ...
            weight (Tensor, optional): of shape (N, C, H, W). Element-wise weights. Default: None.
        """
        level_1 = l1_loss(pred, target, weight, reduction=self.reduction)

        level_2 = l1_loss(pred[:,:,::2,::2], target[:,:,::2,::2], weight, reduction=self.reduction)

        level_3 = l1_loss(pred[:,:,::4,::4], target[:,:,::4,::4], weight, reduction=self.reduction)

        return self.loss_weight * (level_1 + level_2 + level_3) / 3
...
class MultiL1Loss(nn.Module):
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        Args:
            ...
            weight (Tensor, optional): of shape (N, C, H, W). Element-wise weights. Default: None.
        """
        level_1 = l1_loss(pred[:,:,::2,::2], target[:,:,::2,::2], weight, reduction=self.reduction)

        pred_blur = self.gaussian_conv(pred)
        target_blur = self.gaussian_conv(target)

        level_2 = l1_loss(pred - pred_blur, target - target_blur, weight, reduction=self.reduction)

        level_3 = l1_loss(pred, target, weight, reduction=self.reduction)
...
class LaplacianLoss(nn.Module):
    def __init__(self, loss_weight=1.0, reduction='mean'):
        ...
        self.criterion = nn.L1Loss(reduction='mean') # sum change to mean
        self.lap = LaplacianPyramid()
        self.loss_weight = loss_weight

    def forward(self, x, y):
        x_lap, y_lap = self.lap(x), self.lap(y)
        return self.loss_weight * sum(2 ** i * self.criterion(a, b) for i, (a, b) in enumerate(zip(x_lap, y_lap)))
...
class MultiLapLoss(nn.Module):
    ...
    def __init__(self, loss_weight=1.0, reduction='mean'):
        super(MultiLapLoss, self).__init__()
        if reduction not in ['none', 'mean', 'sum']:
            raise ValueError(f'Unsupported reduction mode: {reduction}. Supported ones are: {_reduction_modes}')
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        Args:
            ...
            weight (Tensor, optional): of shape (N, C, H, W). Element-wise weights. Default: None.
        """
        level_1 = self.laploss(pred[:,:,::2,::2], target[:,:,::2,::2])

        pred_blur = self.gaussian_conv(pred)
        target_blur = self.gaussian_conv(target)

        level_2 = self.laploss(pred - pred_blur, target - target_blur)

        level_3 = self.laploss(pred, target)

        return self.loss_weight * (level_2 + level_3)
...
class LRL1Loss(nn.Module):
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        Args:
            ...
            weight (Tensor, optional): of shape (N, C, H, W). Element-wise weights. Default: None.
        """
        pred_LR = F.interpolate(pred, scale_factor=2, mode='bicubic', recompute_scale_factor=True)
        target_LR =F.interpolate(target, scale_factor=2, mode='bicubic', recompute_scale_factor=True)
        return self.loss_weight * l1_loss(pred_LR, target_LR, weight, reduction=self.reduction)


@LOSS_REGISTRY.register()
class histcLoss(nn.Module):

    def __init__(self, loss_weight=1.0, reduction='mean'):
        super(histcLoss, self).__init__()
        if reduction not in ['none', 'mean', 'sum']:
            raise ValueError(f'Unsupported reduction mode: {reduction}. Supported ones are: {_reduction_modes}')
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        """
        _, _, H, W = pred.shape
        pred_LR = torch.histc(pred, bins=128, min=0, max=1)/H/W
        target_LR = torch.histc(target, bins=128, min=0, max=1)/H/W
        print(pred_LR.detach())
        print(target_LR.detach())
        return self.loss_weight * l1_loss(pred_LR.detach(), target_LR.detach(), weight, reduction=self.reduction)
...
class fftLoss(nn.Module):
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        target_fft = fft.fftn(target, dim=(2, 3))
...
class dctLoss(nn.Module):
    ...
    def forward(self, pred, target, weight=None, **kwargs):
        ...
        """
        _, _, H, W = pred.shape
        pred_fft = dct.dct_2d(pred)
        target_fft = dct.dct_2d(target)
