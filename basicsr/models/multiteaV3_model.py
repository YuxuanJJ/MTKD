...
class SRModelTea3(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(SRModelTea3, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output = self.net_g(torch.cat([self.output_tea1, self.output_tea2, self.output_tea3], dim=1))
        self.output = self.output / self.img_range + self.mean.type_as(self.lq)
    ...
    def test(self):
        ...
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                ...
                self.output_tea1 = self.net_g_tea1(img)
                self.output_tea2 = self.net_g_tea2(img)
                self.output_tea3 = self.net_g_tea3(img)

                self.output = self.net_g_ema(torch.cat([self.output_tea1, self.output_tea2, self.output_tea3], dim=1))
                self.output = self.output / self.img_range + self.mean.type_as(self.lq)
        else:
            self.net_g.eval()
            with torch.no_grad():
                ...
                print('------Processed time taken: {:4.6f}'.format(start.elapsed_time(end)))
            self.net_g.train()
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            #t0 = time.time()
            self.test()
            #print('------Processed time taken: {:4.2f}'.format(time.time() - t0))
